#!/usr/bin/env python3
"""
云之家_工会费用报销申请数据同步脚本 (MySQL 版)
功能：从云之家 API 拉取工会费用报销申请数据，写入 MySQL 数据库
"""
...
TABLE_NAME = "云之家_工会费用报销申请"

# 工会费用报销申请表单配置
FORM_CODE_ID = "7013d3cb6c5b465480e229d35ee61c3b"
VIEW_ID = "693771b67365866c5f7c1611"
...
RESULT_ITEMS = [
    ...
    {"sum": False, "title": "关联工会用款单", "type": "relatedWidget", "codeId": "Rd_0"},
    {"sum": False, "title": "事由", "type": "textAreaWidget", "codeId": "Ta_0"},
    {"sum": False, "title": "单据号回写", "type": "fusionFormWidget", "codeId": "Ff_0"},
    {"sum": False, "title": "付讫记录", "type": "checkboxWidget", "codeId": "Cb_0"},
    ...
    {"sum": False, "title": "单位名称（汇款资料）", "type": "basicDataWidget", "codeId": "Bd_9"},
...
# 列名列表（与 CREATE TABLE 顺序一致）
COLUMNS = [
    '流水号', '标题', '制单人', '制单部门', '制单日期', '费报类型', '费报人',
    '制单年度', '费报日期', '发票抬头公司', '费用承担部门',
    '费报总金额', '付款情况', '关联工会用款单', '事由', '单据号回写',
    '付讫记录', '发票附件', '使用类型', '普通类型', '用款总金额',
    '单位名称旧', '开户银行', '单位名称', '银行账号',
    '流程状态', '当前节点', '当前审批人', '审批完成时间',
    '大类目', '费报月份', '费报年度', '云之家ID'
]
...
def fetch_records(cookie, page_size=100, page_number=1, search_items=None):
    """从云之家 API 获取工会费用报销申请数据"""
    headers = {
...
def save_to_database(records):
    ...
    sql = f"""INSERT INTO `{TABLE_NAME}` ({col_names})
              VALUES ({placeholders})
              ON DUPLICATE KEY UPDATE `同步时间` = CURRENT_TIMESTAMP"""
    ...
    for record in records:
        ...
        payment_amount = parse_amount(cols.get("Mo_1"))
        ...
        if att:
            try:
                if isinstance(att, (list, dict)):
                    attachment_str = json.dumps(att, ensure_ascii=False)
        ...
        values = (
            ...
            clean_value(cols.get("Cb_0")),
            attachment_str,
            clean_value(cols.get("Ra_1")),
            clean_value(cols.get("Ra_2")),
            payment_amount,
            clean_value(cols.get("Te_9")),
...
def sync_all():
    """同步所有数据"""
    print("=" * 50)
    print(f"开始同步云之家工会费用报销申请数据到 MySQL")
    print("=" * 50)
    ...
    while True:
        print(f"\n正在获取第 {page_number} 页...")
        result = fetch_records(cookie, page_size=100, page_number=page_number)
...
def sync_by_status(statuses):
    ...
    for status in statuses:
        ...
        page = 1
        while True:
            result = fetch_records(cookie, page_size=100, page_number=page, search_items=single_search)
            if not result.get("success"):
...
def sync_by_date(start_date, end_date=None):
    ...
    result = fetch_records(cookie, search_items=search_items)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="云之家工会费用报销申请数据同步 (MySQL)")
