#!/usr/bin/env python3
"""
云之家付款申请单数据同步脚本 (MySQL 版)
功能：从云之家 API 拉取付款申请单数据，写入 MySQL 数据库
表单：付款申请单（含付款明细子表）
"""
...
# 付款申请单表单配置
FORM_CODE_ID = "3ed2af9eaf7d4c8cb6f08346e38044c2"
VIEW_ID = "697613e8c0c4021d28495b93"

# 要查询的字段（完整 resultItems）
# 注意: Dd_0子表字段必须用 flat 格式+parentCodeId，嵌套 details 不会生效
RESULT_ITEMS = [
    ...
    {"sum": False, "title": "单位类型", "type": "enumWidget", "codeId": "En_4"},
    {"sum": False, "title": "账号", "type": "textWidget", "codeId": "Te_4"},
    # Dd_0 子表字段：用 flat 方式独立请求（parentCodeId=Dd_0）
...
# MySQL 表结构（中文字段名）
CREATE_TABLE_SQL = """CREATE TABLE IF NOT EXISTS `付款单（爱尔康）` (
...
COLUMNS = [
    ...
    '单位类型', '账号', '收款单位类型', '收款单位', '流程状态',
    '单据号回写', '是否传ERP', '当前节点', '当前审批人', '审批完成时间',
    '预算项目', '预算部门', '业务月份', '业务年度', '项目名称', '金额辅助',
    '子表_实付金额', '子表_往来单位类型', '子表_往来单位', '子表_付款用途',
    '单据来源类型', '来源单据流水', '是否生成流水'
]
...
def parse_datetime_timestamp(ts):
    """时间戳转完整日期时间 YYYY-MM-DD HH:MM:SS"""
    if not ts:
        return None
    if isinstance(ts, str):
        try:
...
def save_to_database(records):
    ...
    for record in records:
        ...
        # 获取收款单位 - Mbd_1是多选基础资料，序列化存储
        payee = cols.get("Mbd_1")
        if payee and isinstance(payee, list):
            payee_names = []
            for p in payee:
                if isinstance(p, dict):
                    payee_names.append(p.get("name", str(p)))
                else:
                    payee_names.append(str(p))
            payee_str = ';'.join(payee_names)
        else:
            payee_str = clean_value(payee) if not isinstance(payee, list) else None
        # 解析子表数据
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="云之家付款申请单数据同步 (MySQL)")
